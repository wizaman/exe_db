...
[共通ポリシー](./schme_common.md)も参照。

## ID定義

接頭辞+数値+接尾辞の形式で表し、接頭辞でカテゴリの区別がつくようにします。接尾辞はマイナーチェンジが必要なときにつけます。数値は各区分に合わせて桁数は揃えます（例えばバトルチップは3桁確保してゼロパディング）

接頭辞・接尾辞は下記のように整理されます。
...
| GroupNo    | int      | SortIdの一部に採用されるグループ番号です。省略時（0）はNoを採用してSortIdを算出しています。<br />データライブラリ上の番号は同じでもゲームバージョンが違うものについて、番号別にゲームバージョンでソートすると順序が混在するため、グループ化してソート順を調整する意図があります。 |
